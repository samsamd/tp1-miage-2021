...
#DEFOSSE Samuel et THIVET Simon#
...
Non le code n'est pas structuré en couches, cela va poser un problème de maintenabilité lors de l'évolution du code.

*Où se trouve le code métier (voir la règle de gestion RG 1) ?*
Le code métier se trouve dans le fichier TodoListController.java

*Cette règle est-elle facilement testable par un test unitaire ?*
Cette règle est difficilement testable puisque la méthode est privé donc il faudrait effectuer le TU dans le controller.
...
La commande docker run installe le conteneur, l'option POSTGRES_PASSWORD permet de définir la variable d'environnement et -p permet de définir un numéro de port pour accéder au conteneur.

* Compléter le code manquant dans la méthode `TodoListController.createTodoItem()`
Voir ligne 42 dans TodoListController.java

*Pourquoi `todoItemRepository` est-il `null` ? Quelle est la meilleure façon de l'injecter ?*
C'est parce qu'il n'est pas injecté. On remplace la déclaration de 'todoItemRepository' par une injection de celle-ci à l'aide de l'annotation @Inject.
...
L'architecture hexagonale permet de séparer le code en ensembles fortement cohérents et faiblement coupés afin d'éviter la dépendance entre plusieurs couches tout en respectant les principes du SOLID. Cela assure une bonne maintenabilité, testabilité et structuration du projet.
...
Ce sont des tests d'intégrations
*Que teste-on dans ce cas ?*
On teste que l'ensemble de l'application fonctionne une fois tous les modules assemblés
